...
- **安定性改善**: Relay の再接続と認証状態の保持を修正し、Alexa からのコマンド実行の信頼性を向上させました。
- **遅延改善**: AI マッチングのキャッシュ経路を強化し、キャッシュヒット時の応答を高速化しました。
- **計測の可視化**: `Auth Verification` 内訳に `Auth UserInfo` と `Auth Token Lookup` を追加し、遅延原因の特定が容易になりました。
...
- **\[推奨] Stream Deck Marketplace から入手**
  ...
  _(※現在公開申請中です。公開後にこちらから直接インストール可能になります)_
